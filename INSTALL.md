...
agent-redline is at **v0.1**. The skill works end-to-end on JVM repos (Spring covered explicitly via an addendum) through the `jvm-archunit` reference extension. Other ecosystems get zone classification, checkpoints, and PR-size checks but no boundary-rule enforcement (see [README.md](README.md#what-v01-ships) for the full surface).
