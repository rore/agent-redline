...
**Suppression markers** — copy the extension's `suppressions.yaml` (or `core/templates/suppressions.yaml` if zone-only) to `.agent-redline/suppressions.yaml`. Add to policy: `suppressions: { useExtensionDefaults: true, exemptPaths: ["**/tests/**"] }`. Re-bootstrap over a v0.2 policy without this block: ask; decline leaves it off.
