...
Each subdirectory under `pr-scenarios/` describes one of the three planned PR branches in `agent-redline-demo`. Each scenario has:
...
`scripts/sync-demo.sh --with-pr-branches` runs the apply scripts to produce the three branches.

## The three scenarios

| Scenario | Branch | Verdict |
|---|---|---|
| Blue-only DTO field add | `demo/blue-only-pr` | `BLUE`, exit 0 |
