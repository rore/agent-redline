...
- Bootstrap-mode gets a JVM shape triage table with Java/Kotlin web framework, library, and zone-only signals.
...
- Two new bootstrap-detect fixtures (`jvm-spring-service`, `jvm-library`) prove the shape detection in code, not just markdown.
...
- Existing dated decision blocks (including "v0.1 ships with `spring-archunit` only") stay as historical record. SPEC §15.1 has a one-line "superseded" note pointing here.
