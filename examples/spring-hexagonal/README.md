...
A minimal Spring Boot service in hexagonal layout. Used inside the agent-redline repo as the **Layer 3 extension scaffold dry-run target** (see `tests/extensions/jvm-archunit/check-extension.sh` and `docs/VALIDATION.md`).
...
The Layer 3 harness (`tests/extensions/jvm-archunit/check-extension.sh` in the agent-redline repo) runs both, then injects a deliberate boundary violation, verifies the right rule fails, and restores the fixture.

## See also

- `agent-redline-demo` repo — the runnable, paired demo repo with policy, CI, and three planned PRs
- `extensions/jvm-archunit/` in agent-redline — the language extension this fixture is shaped for
